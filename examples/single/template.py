...
class SynthSingle(templates.Template):
    ...
    def generate(self):
        ...
        fg_image, label, bboxes, glyph_fg_image, glyph_bboxes,text = self._generate_text(
            fg_color, fg_style
        )
        ...
        if midground:
            mg_image, *_ = self._generate_text(mg_color, mg_style)
            mg_image = self._erase_image(mg_image, fg_image)
            bg_image = _blend_images(mg_image, bg_image, False)#self.visibility_check)
    ...
    def _generate_text(self, color, style):
        ...
        return out, label, bboxes, glyph_out, glyph_bboxes,text
